<?php
    ...
    $old_password= md5(md5(mysqli_real_escape_string($connect,$_POST['oldPassword'])));
    $new_password= md5(md5(mysqli_real_escape_string($connect,$_POST['newPassword'])));
    $email=$_SESSION['email'];
    //echo $email;
    $password_from_database_query="select password from users where email='$email'";
    $password_from_database_result=mysqli_query($connect,$password_from_database_query) or die(mysqli_error($connect));
